...
export class DiseaseComponentApp {
	...
	*-------------------
    ...
    getVersionId(){
    	this.dataTreeService.getVersion()
    	.then( res=>{
	...
	getDataTree(){
		this.dataTreeService.getDrugsTree()
			.then(res => {
				if(res.code == 200){
	 ...
	 /*版本管理*/
    versionControl(){
    	let link:any= ['page/all_version_management/Sickness_version_management'];
	...
	*更新修改后的保存逻辑
	...
	getSearchResult(dictValue:string){
		...
		this.dataTreeService.getSearchTree(dictValue)
			.then(res => {
				if (res != "") {
					if(!dictValue){
